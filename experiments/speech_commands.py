...
def main(device='cuda', max_epochs=200, *,                                        # training parameters
         ...
         dry_run=False, dropped_rate = 0, raw_data=False, **kwargs):                                                               # kwargs passed on to cdeint
    ...
    times, train_dataloader, val_dataloader, test_dataloader = datasets.speech_commands.get_data(intensity_data,
                                                                                                 batch_size, dropped_rate, raw_data)
    if raw_data:
        input_channels = 1 + (1 + intensity_data) * 1
    else:
...
def run_all(device, model_names=('ncde', 'odernn', 'dt', 'decay', 'gruode'), dropped_rate=0, raw_data=False):
    ...
    for model_name in model_names:
        ...
        for _ in range(5):
            main(device, model_name=model_name, **model_kwargs[model_name], dropped_rate=dropped_rate, raw_data=raw_data)
